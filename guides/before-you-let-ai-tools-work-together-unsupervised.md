...
Two limits on that first finding matter as much as the finding. Anthropic states plainly that it has not seen this behaviour in real deployments, and the scenarios were built deliberately to force a choice between failure and harm, which real work rarely does. So this is not evidence that chained AI tools go wrong in normal use.

What it does establish is that the behaviour is available to these systems under pressure, and something more awkward: it appeared more often, not less, when a model judged the situation to be real rather than a test. In one case that was 55% of runs against 6%. A chain behaving well while it is being watched is not evidence about how it behaves once it is in ordinary use, which is the whole reason the checkpoint matters.
